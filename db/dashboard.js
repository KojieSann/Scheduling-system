  ...
  //search function for the table
    ...
    searchInput.addEventListener('input', function() {
      ...
      tableRows.forEach(function(row) {
        ...
        const schedule = row.querySelector('td:nth-child(3)').textContent.toLowerCase().trim();
        ...
        const time = row.querySelector('td:nth-child(6)').textContent.toLowerCase().trim();
        const adviser = row.querySelector('td:nth-child(7)').textContent.toLowerCase().trim();

        if (section.includes(searchTerm) || strand.includes(searchTerm) || schedule.includes(searchTerm) || sem.includes(searchTerm) || sy.includes(searchTerm) || time.includes(searchTerm) || adviser.includes(searchTerm)) {
